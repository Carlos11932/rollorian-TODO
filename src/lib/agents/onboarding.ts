...
function createEnvBlock(baseUrl: string, token: string) {
  return [
    `ROLLORIAN_TODO_BASE_URL=${baseUrl}`,
    `ROLLORIAN_TODO_AGENT_TOKEN=${token}`,
  ].join("\n");
}
...
export function buildAgentOnboardingSnippets({
  ...
  const pathToMcp = `${repoRootPlaceholder}/mcp/rollorian-todo-mcp`;
  ...
  return [
    {
      ...
      primarySnippet: [
        `codex mcp add ${serverName} --env ROLLORIAN_TODO_BASE_URL=${baseUrl} --env ROLLORIAN_TODO_AGENT_TOKEN=${resolvedToken} -- npm --prefix ${pathToMcp} run dev:stdio`,
        "codex mcp list",
      ].join("\n"),
      secondaryLabel: "env",
    ...
    },
    {
      ...
      primaryLabel: "command",
      primarySnippet: [
        `claude mcp add --transport stdio --env ROLLORIAN_TODO_BASE_URL=${baseUrl} --env ROLLORIAN_TODO_AGENT_TOKEN=${resolvedToken} ${serverName} -- npm --prefix ${pathToMcp} run dev:stdio`,
        "claude mcp list",
      ].join("\n"),
      secondaryLabel: "env",
      secondarySnippet: envBlock,
    ...
    },
    {
      ...
      secondaryLabel: "curl",
      secondarySnippet: `curl -H "Authorization: Bearer ${resolvedToken}" ${baseUrl}/api/agent/v1/views/my`,
    },
  ];
}
