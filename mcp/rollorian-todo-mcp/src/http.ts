...
export async function startHttpServer(): Promise<void> {
  const host = process.env.ROLLORIAN_TODO_MCP_HTTP_HOST ?? "127.0.0.1";
  const port = Number(process.env.ROLLORIAN_TODO_MCP_HTTP_PORT ?? "8789");
  const app = express();
  ...
  await new Promise<void>((resolve, reject) => {
    const server = app.listen(port, host, (error?: Error) => {
      ...
      process.stdout.write(`Rollorian Todo MCP listening on http://${host}:${port}/mcp\n`);
      resolve();
    });
